...
public class ComputerScienceShortAttendance extends JFrame {
	MyConnectionClass m;
	JTable tab;
	JScrollPane jsp;
	String rows[]= {"S. No.","Student Name","Student ID","Total Attendance"};
	String d[][];
	ComputerScienceShortAttendance()
	{
		super("STUDENT MANAGEMENT SYSTEM- Computer Science Attendance(Students with Short Attendance)");
	...
	void getData()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(temp.equals("Computer Science 102")&&i1<=45)
				{
					temp="";
					temp+=sno;
					d[i][j++]=temp;
