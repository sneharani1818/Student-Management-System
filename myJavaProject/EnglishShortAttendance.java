...
public class EnglishShortAttendance extends JFrame {
	MyConnectionClass m;
	JTable tab;
	JScrollPane jsp;
	String rows[]= {"S. No.","Student Name","Student ID","Total Attendence"};
	String d[][];
	EnglishShortAttendance()
	{
		super("STUDENT MANAGEMENT SYSTEM- English Attendance(Students with Short Attendance)");
	...
	void getData()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(temp.equals("English 101")&&i1<=45)
				{
					temp="";
				temp+=sno;
					d[i][j++]=temp;
