...
public class ElectronicsAttendance extends JFrame {
	MyConnectionClass m;
	JTable tab;
	JScrollPane jsp;
	String rows[]= {"S. No.","Student Name","Student ID","Total Attendance"};
	String d[][];
	ElectronicsAttendance()
	{
		super("STUDENT MANAGEMENT SYSTEM- Electronics Attendance");
	...
	void getData()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(temp.equals("Electronics 106"))
				{
					temp="";
				temp+=sno;
					d[i][j++]=temp;
