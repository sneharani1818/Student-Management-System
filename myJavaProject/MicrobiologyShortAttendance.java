...
public class MicrobiologyShortAttendance extends JFrame {
	MyConnectionClass m;
	JTable tab;
	JScrollPane jsp;
	String rows[]= {"S. No.","Student Name","Student ID","Total Attendance"};
	String d[][];
	MicrobiologyShortAttendance()
	{
		super("STUDENT MANAGEMENT SYSTEM- MicroBiology Attendance(Students with Short Attendance)");
	...
	void getData()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(temp.equals("Microbiology 108"))
				{
					temp="";
				temp+=sno;
					d[i][j++]=temp;
