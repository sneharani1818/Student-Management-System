...
public class BioMedicalScienceShortAttendance extends JFrame {
	MyConnectionClass m;
	JTable tab;
	JScrollPane jsp;
	String rows[]= {"S. No.","Student Name","Student ID","Total Attendance"};
	String d[][];
	BioMedicalScienceShortAttendance()
	{
		super("STUDENT MANAGEMENT SYSTEM- BioMedical Science Attendance(Students with Short Attendance)");
	...
	void getData()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(temp.equals("BioMedical Science 109")&&i1<=45)
				{
					temp="";
				temp+=sno;
					d[i][j++]=temp;
