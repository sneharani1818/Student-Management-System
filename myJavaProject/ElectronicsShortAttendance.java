...
public class ElectronicsShortAttendance extends JFrame {
	MyConnectionClass m;
	JTable tab;
	JScrollPane jsp;
	String rows[]= {"S. No.","Student Name","Student ID","Total Attendance"};
	String d[][];
	ElectronicsShortAttendance()
	{
		super("STUDENT MANAGEMENT SYSTEM- Electronics Attendance(Students with Short Attendance)");
	...
	void getData()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(temp.equals("Electronics 106")&&i1<=45)
				{
					temp="";
				temp+=sno;
					d[i][j++]=temp;
