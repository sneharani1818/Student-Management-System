...
public class MathematicsShortAttendance extends JFrame {
	MyConnectionClass m;
	JTable tab;
	JScrollPane jsp;
	String rows[]= {"S. No.","Student Name","Student ID","Total Attendence"};
	String d[][];
	MathematicsShortAttendance()
	{
		super("STUDENT MANAGEMENT SYSTEM- Mathematics Attendance(Students with Short Attendance)");
	...
	void getData()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(temp.equals("Mathematics 103")&&i1<=45)
				{
					temp="";
				temp+=sno;
					d[i][j++]=temp;
