...
public class FoodTechShortAttendance extends JFrame {
	MyConnectionClass m;
	JTable tab;
	JScrollPane jsp;
	String rows[]= {"S. No.","Student Name","Student ID","Total Attendance"};
	String d[][];
	FoodTechShortAttendance()
	{
		super("STUDENT MANAGEMENT SYSTEM- Food Technology Attendance(Students with Short Attendance)");
	...
	void getData()
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				if(temp.equals("FoodTech 107")&&i1<=45)
				{
					temp="";
				temp+=sno;
					d[i][j++]=temp;
